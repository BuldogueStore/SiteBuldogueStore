                      ...
                      {bots.map(bot => (
                        <tr key={bot.id} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <button onClick={() => deleteBot(bot.id)} className="text-gray-500 hover:text-red-600 transition-colors">
        ...
        {isAuthModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="relative bg-zinc-950 border border-red-600/30 w-full max-w-md cyber-border overflow-hidden shadow-[0_0_100px_rgba(220,38,38,0.3)]"
            >
              <div className="tech-scan" />
              <div className="p-10 space-y-8 relative z-10">
                <div className="text-center space-y-2">
                  ...
                  <h3 className="text-3xl font-black uppercase italic tracking-tighter glitch-text">
                    {authMode === 'login' ? 'Sistema de Login' : 'Novo Registro'}
                  </h3>
                  <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-widest">
                    {authMode === 'login' ? 'Insira suas credenciais de acesso' : 'Crie sua conta na plataforma'}
                  </p>
                </div>

                <form onSubmit={handleAuthSubmit} className="space-y-6">
                  {authError && (
                    <div className="bg-red-900/20 border border-red-600/50 p-4 cyber-border text-xs text-red-500 font-mono">
                      {authError}
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest font-mono">Endereço de E-mail</label>
                      <input 
                        type="email" 
                        required
                        disabled={authLoading}
                        className="w-full bg-black border border-zinc-800 px-4 py-3 focus:border-red-600 outline-none transition-colors cyber-border font-mono text-sm"
                        value={authForm.email}
                        onChange={e => setAuthForm({...authForm, email: e.target.value})}
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest font-mono">Senha de Acesso</label>
                      <input 
                        type="password" 
                        required
                        disabled={authLoading}
                  ...
                  <button 
                    type="submit"
                    disabled={authLoading}
                    className="w-full bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-black py-4 cyber-border font-black uppercase text-xs tracking-[0.2em] transition-all shadow-lg active:scale-95 flex items-center justify-center gap-3"
                  >
                    {authLoading ? (
                      <div className="w-4 h-4 border-2 border-black border-t-transparent rounded-full animate-spin" />
                    ) : (
                      authMode === 'login' ? 'Autenticar' : 'Registrar'
                    )}
                  </button>
                </form>

                <div className="text-center border-t border-zinc-900 pt-6">
                  <button 
                    type="button"
                    onClick={() => {
                    ...
                    className="text-xs text-zinc-500 hover:text-red-600 font-mono uppercase tracking-widest transition-colors"
                  >
                    {authMode === 'login' ? '> Criar nova conta <' : '> Já tenho uma conta <'}
                  </button>
                </div>
              </div>
            </motion.div>
